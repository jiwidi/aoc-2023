    ...
    parsed_games = [
        {
            ...
            "draws": [
                {color.split(' ')[1]: int(color.split(' ')[0]) for color in draw.split(', ')}
...
def puzzle_one(games):
    ...
    id_sum = sum(game["game"] for game in games if not any(
        number > available_cubes[color] for draw in game["draws"] for color, number in draw.items()
    ))
    return id_sum

def puzzle_two(games):
    global_game_power = sum(
        math.prod(max(draw.get(color, 0) for draw in game["draws"]) for color in ["red", "green", "blue"])
        for game in games
    )
